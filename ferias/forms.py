...
class SolicitacaoFeriasForm(forms.ModelForm):
    periodo_aquisitivo = forms.ModelChoiceField(
        queryset=PeriodoAquisitivo.objects.none(), 
        label="Período Aquisitivo para Desconto",
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.user:
            # Filtra os períodos para mostrar apenas os do usuário logado que têm saldo
            self.fields['periodo_aquisitivo'].queryset = PeriodoAquisitivo.objects.filter(
    ...
    def celan(self):
        ...
        data_inicio = cleanned_data.get("data_inicio")
        data_fim = cleanned_data.get("data_fim")
        periodo_selecionado = cleanned_data.get("periodo_aquisitivo")

        if not (data_inicio and data_fim and periodo_selecionado):
            return cleanned_data
        
        if data_fim <data_inicio:
            raise ValidationError("A data de término não pode ser anterior à data de inicio.")
        
        total_dias_solicitados = (data_fim - data_inicio).days + 1

        if total_dias_solicitados < 10:
            raise ValidationError("O período para a solicitação de férias é de 10 dias.")
